...
SongService.getAllSongs = async (queryOptions) => {
  try {
    ...
    return list.map((song) => song.toJSON());
  } catch (error) {
 ...
 * @param {date} release_date - Fecha de publicación..
 * @param {string} file_path - Ruta de la canción.
 * @param {boolean} is_published - Indica si la canción está publicada.
 * @returns {Promise<Object>} Objeto de la canción creada.
 ...
 */
SongService.updateSong = async (id, title, is_published) => {
  try {
    const [rowsUpdated] = await SongModel.update(
      { title, is_published },
      {
        where: {
          id,
